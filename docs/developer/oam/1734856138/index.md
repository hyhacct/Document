---
title: "运维"
titleTemplate: categref心跳失败
---

# 概述

之前在工作中碰到一个问题，就是在容器环境中运行夜莺监控的`categref`采集器会出现心跳失败的情况，如下：
...
在这里出现了一个`NaN`的报错，然后我在GitHub上提交了一个[issue](https://github.com/flashcatcloud/categraf/issues/1106)，并且得到了解决方案
...
按照大佬所说的，我在容器启动配置中将`HOST_PROC`这个配置删掉了，然后删除容器重建，再次查看，心跳正常，问题成功解决！
